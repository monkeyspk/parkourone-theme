(function(wp) {
	...
	const { PanelBody, SelectControl, RangeControl, ColorPalette, TextControl } = wp.components;
	const { createElement: el } = wp.element;

	// Verfügbare Icons (Lucide-kompatibel)
	const availableIcons = [
		{ label: 'Stern', value: 'star' },
		{ label: 'Herz', value: 'heart' },
		{ label: 'Benutzer', value: 'user' },
		{ label: 'Benutzer-Gruppe', value: 'users' },
		{ label: 'Kalender', value: 'calendar' },
		{ label: 'Uhr', value: 'clock' },
		{ label: 'Standort', value: 'map-pin' },
		{ label: 'Telefon', value: 'phone' },
		{ label: 'E-Mail', value: 'mail' },
		{ label: 'Check', value: 'check' },
		{ label: 'Check-Kreis', value: 'check-circle' },
		{ label: 'Pfeil rechts', value: 'arrow-right' },
		{ label: 'Pfeil links', value: 'arrow-left' },
		{ label: 'Externer Link', value: 'external-link' },
		{ label: 'Download', value: 'download' },
		{ label: 'Play', value: 'play' },
		{ label: 'Ziel', value: 'target' },
		{ label: 'Award', value: 'award' },
		{ label: 'Zap (Blitz)', value: 'zap' },
		{ label: 'Trend Up', value: 'trending-up' },
		{ label: 'Schild', value: 'shield' },
		{ label: 'Lächeln', value: 'smile' },
		{ label: 'Haus', value: 'home' },
		{ label: 'Info', value: 'info' },
		{ label: 'Hilfe', value: 'help-circle' }
	];
	...
	registerBlockType('parkourone/po-icon', {
		edit: function(props) {
			const { attributes, setAttributes } = props;
			const blockProps = useBlockProps({
				className: 'po-icon-editor',
				style: { textAlign: attributes.align }
			});

			var iconPath = iconPaths[attributes.icon] || iconPaths['star'];
			...
			return el('div', null, [
				...
				el(InspectorControls, { key: 'controls' }, [
					el(PanelBody, { key: 'icon', title: 'Icon-Auswahl', initialOpen: true }, [
						el(SelectControl, {
							key: 'iconSelect',
							label: 'Icon',
						...
						el('p', { key: 'colorLabel', style: { marginBottom: '8px', fontWeight: 500 } }, 'Farbe'),
						el(ColorPalette, {
							key: 'color',
							colors: colors,
							value: attributes.color,
							onChange: function(v) { setAttributes({ color: v || '#667eea' }); }
						})
					]),
