...
foreach ($all_coaches as $coach) {
	...
	$coach_data = [
		...
		'ausbildungen' => get_post_meta($coach_id, '_coach_ausbildungen', true),
		'instagram' => get_post_meta($coach_id, '_coach_instagram', true),
		'website' => get_post_meta($coach_id, '_coach_website', true),
		'hero_bild' => $hero_bild,
		'locations' => $locations
	];
	...
	<div class="po-overlay__panel po-overlay__panel--coach">
		...
		<div class="po-tg-coach">
			...
			<dl class="po-tg-coach__facts">
				...
				<?php endif; ?>
				<?php if (!empty($m['ausbildungen'])): ?>
				<div class="po-tg-coach__fact po-tg-coach__fact--full">
					<dt>Ausbildungen</dt>
					<dd><?php echo nl2br(esc_html($m['ausbildungen'])); ?></dd>
				</div>
				<?php endif; ?>
			</dl>
			<?php endif; ?>

			<?php if (!empty($m['instagram']) || !empty($m['website'])): ?>
			<div class="po-tg-coach__links">
				<?php
				if (!empty($m['instagram'])) {
					$ig = $m['instagram'];
					$ig_url = (strpos($ig, 'http') === 0) ? $ig : 'https://instagram.com/' . ltrim($ig, '@');
					$ig_handle = (strpos($ig, '@') === 0) ? $ig : '@' . basename(rtrim($ig_url, '/'));
					echo '<a href="' . esc_url($ig_url) . '" target="_blank" rel="noopener" class="po-tg-coach__link po-tg-coach__link--instagram">';
					...
					echo ' ' . esc_html($ig_handle);
					echo '</a>';
				}
				if (!empty($m['website'])) {
					echo '<a href="' . esc_url($m['website']) . '" target="_blank" rel="noopener" class="po-tg-coach__link po-tg-coach__link--website">';
					echo '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>';
					echo ' ' . esc_html(preg_replace('#^https?://(www\.)?#', '', rtrim($m['website'], '/')));
					echo '</a>';
				}
				?>
			</div>
